...
permutation_test <- function(population, n, events, obs, start_year){
  ...
  result <- numeric(N)
  for (i in 1:1){
    index <- sample(length(population), size=n, replace=FALSE) #sample from the population the size of all medals won by group of interest
    result[i] <- length(population[index] %>% filter(Event %in% events)) #number of medals won in "events" by any country
  }
  p_val <- (sum(result >= obs) + 1)/(N+1)
  p_val
}
"Likelihood number of distance medals won by East Africans was by chance:"
permutation_test(data %>% filter(Year >= 1956), length(east_african_all), distance_events_all, length(east_afr_distance), 1956)
"Likelihood number of sprint medals won by Americans was by chance:"
permutation_test(data, length(american_all), sprint_events_all, length(american_sprints), 1896)
